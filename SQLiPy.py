...
class SqlMapScanIssue(IScanIssue):
    ...
    def __init__(self, httpService, url, httpMessages, name, detail, confidence, severity):
        ...
        with open('file.txt', 'w') as file:
            file.write(HttpService)
	    file.write(url)
	    file.write(httpMessages)
	    file.write(name)
	    file.write(detail)
	    file.write(severity)
        return
...
class BurpExtender(IBurpExtender, IContextMenuFactory, ITab, IExtensionStateListener):
  ...
  def sqlMapScan(self, invocation):
	...
	if re.search('[a-zA-Z0-9]', bodyData) is not None:
            postdata = bodyData
            datacmd = ' --data="' + bodyData + '"'

        if re.search('[a-zA-Z0-9]', cookie) is not None:
            cookiedata = cookie
            cookiecmd = ' --cookie="' + cookie + '"'

        if re.search('[a-zA-Z0-9]', ua) is not None:
          uadata = ua
          uacmd = ' --user-agent="' + ua + '"'

        if re.search('[a-zA-Z0-9]', referer) is not None:
          refererdata = referer
